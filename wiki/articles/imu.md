...
LiDAR-Inertial SLAM では、IMU で scan 間の初期姿勢推定とデスキュー (歪み補正) を行う。`FAST_LIO` は iterated Kalman Filter で IMU と LiDAR を tightly-coupled に融合し、高速な LIO を実現。`li_slam_ros2` も NDT/GICP + IMU の tightly-coupled 構成。
...
IMU の誤差はバイアス (ゆっくりドリフト) とホワイトノイズに分解される。Allan Variance で特性評価し、フィルタのプロセスノイズに反映する。SLAM Handbook (slam-handbook-python, slam-handbook-public-release) でこの理論を学べる。

## 関連論文

- An Enhanced LiDAR-Inertial SLAM System (2022): ロボット localization 向け改良型 LIO
- OKVIS2-X (2025): Visual-Inertial SLAM に LiDAR と GNSS を統合
